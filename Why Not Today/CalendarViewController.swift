...
extension CalendarViewController: JTAppleCalendarViewDataSource {
    func configureCalendar(_ calendar: JTAppleCalendarView) -> ConfigurationParameters {
        ...
        let params = ConfigurationParameters(startDate: startDate, endDate: Date(), numberOfRows: 6,
                calendar: nil, generateInDates: nil, generateOutDates: .tillEndOfRow, firstDayOfWeek: nil, hasStrictBoundaries: nil)
...
extension CalendarViewController: JTAppleCalendarViewDelegate {
    func calendar(_ calendar: JTAppleCalendarView, willDisplay cell: JTAppleCell, forItemAt date: Date, cellState: CellState, indexPath: IndexPath) {
        ...
        changeCellDisplay(customCell, with: calendarView, withState: cellState)
        customCell.selectedView.isHidden = Calendar.current.startOfDay(for: date) != selectedDate
        customCell.progressView.isHidden = true
        customCell.failedProgressView.isHidden = true
        customCell.failedProgressView.transform = CGAffineTransform.init(scaleX: -1, y: 1)

        let percent = getProgressBarPercentage(forDate: date)
        if percent.0 != nil {
            customCell.progressView.isHidden = false
            customCell.progressView.setProgress(percent.0!, animated: false)
        }
        if percent.1 != nil {
            customCell.failedProgressView.isHidden = false
            customCell.failedProgressView.setProgress(percent.1!, animated: false)
        }
    }
